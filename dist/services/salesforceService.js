...
exports.createSalesforceService = createSalesforceService;
...
class SalesforceService {
     ...
     */
    async initConnection(accessToken, refreshToken, instanceUrl) {
        ...
        this.connection.on('refresh', (newAccessToken) => {
    ...
    async syncAccount(account) {
        ...
        try {
            ...
            if (account.Website) {
                const query = await this.connection.query(`SELECT Id FROM Account WHERE Website = '${account.Website}' LIMIT 1`);
                existingAccount = query.records[0];
            }
            if (!existingAccount) {
                const nameQuery = await this.connection.query(`SELECT Id FROM Account WHERE Name = '${account.Name.replace(/'/g, "\\'")}' LIMIT 1`);
    ...
    async syncContact(contact) {
        ...
        try {
            ...
            if (contact.Email) {
                const query = await this.connection.query(`SELECT Id FROM Contact WHERE Email = '${contact.Email}' LIMIT 1`);
     ...
     */
    async exportAccountPackage(pkg) {
        if (!this.connection) {
            return {
                success: false,
    ...
    async importAccounts(query) {
        ...
        LIMIT 1000
      `;
            const result = await this.connection.query(query || defaultQuery);
            return {
                success: true,
     ...
     */
    async importContactsForAccount(accountId) {
        if (!this.connection) {
    ...
    async subscribeToAccountChanges(callback) {
        ...
        const subscription = this.connection.streaming.topic(channel).subscribe((message) => {
            callback({
 ...
 */
function createSalesforceService(config, tenantId) {
