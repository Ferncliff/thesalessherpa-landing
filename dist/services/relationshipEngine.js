...
exports.createRelationshipEngine = createRelationshipEngine;
/**
...
class RelationshipEngine {
     ...
     */
    findPaths(targetId, maxDepth = 7, maxPaths = 3) {
        ...
        while (queue.length > 0 && allPaths.length < maxPaths * 2) {
            ...
            for (const edge of edges) {
                if (current.visited.has(edge.targetId))
                    continue;
     ...
     */
    findAllReachable(maxDegrees = 4) {
        const result = new Map();
        const visited = new Set();
        const currentLevel = new Set([this.userId]);
     ...
     */
    calculateIntroSuccessRate(path) {
        if (path.length < 2)
            return 0;
        // Base success rate by degrees of separation
        const degreeSuccessRates = {
            1: 0.85, // Direct intro
            2: 0.70, // One hop
            3: 0.55, // Two hops
            4: 0.40, // Three hops
        ...
        let typeBonus = 0;
        const strongTypes = ['colleague', 'former_colleague', 'mentor', 'mentee', 'manager'];
        const mediumTypes = ['classmate', 'friend', 'partner'];
        for (const hop of path) {
     ...
     */
    suggestBestConnectors(targetId) {
     ...
     */
    findWarmIntrosForAccount(contactIds) {
     ...
     */
    analyzeRelationship(nodeId1, nodeId2) {
 ...
 */
async function createRelationshipEngine(userId, connections, secondDegreeConnections) {
...
exports.default = RelationshipEngine;
