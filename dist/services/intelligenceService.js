...
exports.createIntelligenceService = createIntelligenceService;
...
class IntelligenceService {
     ...
     */
    async generateActionRecommendations(account) {
     ...
     * Analyze relationship opportunities
     */
    analyzeRelationshipOpportunities(account) {
        const actions = [];
     ...
     */
    analyzeTimingOpportunities(account) {
        const actions = [];
        const now = new Date();
        // Check for stale contacts
        for (const contact of account.contacts) {
            if (contact.lastContacted) {
                const daysSinceContact = Math.floor((now.getTime() - new Date(contact.lastContacted).getTime()) / (1000 * 60 * 60 * 24));
     ...
     */
    analyzeAlertOpportunities(account) {
        const actions = [];
     ...
     */
    async getAIRecommendations(account) {
        try {
     ...
     */
    buildRecommendationPrompt(account) {
        return `
...
Provide 2-3 specific, actionable recommendations in this JSON format:
{
  "recommendations": [
     ...
     */
    async generateOutreachMessage(context) {
        try {
            const response = await this.openai.chat.completions.create({
                model: this.config.model || 'gpt-4-turbo-preview',
                messages: [
...
Best regards`,
     ...
     */
    async analyzeNewsSignal(article) {
        try {
            const response = await this.openai.chat.completions.create({
                model: this.config.model || 'gpt-4-turbo-preview',
                messages: [
...
Return JSON:
{
  ...
  "confidence": 0.0-1.0
     ...
     */
    async getDashboardIntelligence(accounts) {
     ...
     */
    async generateWeeklyDigest(accounts, userName) {
 ...
 */
function createIntelligenceService(config) {
