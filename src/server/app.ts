...
import { calculateUrgencyScore, getPriorityLevel, ScoringBreakdown } from './services/urgencyScoring';
import { RelationshipEngine, createRelationshipEngine } from './services/relationshipEngine';
import { SalesforceService, createSalesforceService } from './services/salesforceService';
...
function loadMattAccounts() {
  try {
    const filePath = join(process.cwd(), 'data/accounts/matt_fa_accounts_with_ats.json');
    ...
    return data.accounts.map((account: any) => ({
      ...
      contacts: account.relationships?.map((rel: any, index: number) => ({
      ...
      alerts: account.insights?.map((insight: any, index: number) => ({
...
// Get all accounts
app.get('/api/accounts', optionalAuth, (req: AuthenticatedRequest, res: Response) => {
  ...
  const accountsWithScoring = sortedAccounts.map(account => {
    const breakdown = calculateUrgencyScore({
      id: account.id,
      name: account.name,
      ...
      alerts: account.alerts.map(a => ({ ...a, type: a.type as any, urgency: a.urgency as any })),
      activities: account.activities.map(a => ({ ...a, id: uuidv4(), createdAt: a.date })),
      lastActivityAt: account.lastActivityAt
    });

    const priority = getPriorityLevel(breakdown.overall);
  ...
  res.json({
    ...
    meta: {
      ...
      avgUrgencyScore: Math.round(
        accountsWithScoring.reduce((sum, acc) => sum + acc.urgencyScore, 0) / accountsWithScoring.length
      )
    }
  });
});
...
// Get single account
app.get('/api/accounts/:id', optionalAuth, (req: AuthenticatedRequest, res: Response) => {
  const mattAccounts = loadMattAccounts();
  const allAccounts = mattAccounts.length > 0 ? mattAccounts : demoAccounts;
  const account = allAccounts.find((a: any) => a.id === req.params.id);
...
app.get('/api/accounts/:id/relationships', optionalAuth, async (req: AuthenticatedRequest, res: Response) => {
  ...
  const engine = await createRelationshipEngine(
    'demo-user',
    [
...
app.get('/api/intelligence/dashboard', optionalAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    // Load Matt's real accounts and convert to intelligence context
    const mattAccounts = loadMattAccounts();
    const allAccounts = mattAccounts.length > 0 ? mattAccounts : demoAccounts;
    const accountContexts = allAccounts.map((acc: any) => ({
      id: acc.id,
      name: acc.name,
    ...
    const avgScore = Math.round(
      accountContexts.reduce((sum, a) => sum + a.urgencyScore, 0) / accountContexts.length
    );
...
// Get alerts
app.get('/api/intelligence/alerts', optionalAuth, (req: AuthenticatedRequest, res: Response) => {
  const mattAccounts = loadMattAccounts();
  const allAccounts = mattAccounts.length > 0 ? mattAccounts : demoAccounts;
  const allAlerts = allAccounts.flatMap((account: any) =>
...
app.get('/api/cadence/matrix', optionalAuth, (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const cadenceData = allAccounts.slice(0, 8).map((account: any) => ({
      ...account,
      contacts: (account.contacts || []).map((contact: any) => ({
        ...contact,
...
app.post('/api/intelligence/outreach', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
app.get('/api/relationships/linkedin', optionalAuth, (req: AuthenticatedRequest, res: Response) => {
  try {
    const intelligencePath = join(process.cwd(), 'data/processed/matt_linkedin_intelligence.json');
    const connectionsPath = join(process.cwd(), 'data/processed/matt_linkedin_connections.json');
    
    let intelligence = {};
    let connections = [];
    
    try {
      intelligence = JSON.parse(readFileSync(intelligencePath, 'utf8'));
...
app.get('/api/relationships/network', optionalAuth, async (req: AuthenticatedRequest, res: Response) => {
  const engine = await createRelationshipEngine(
    'demo-user',
    [
      { id: 'sarah-johnson', name: 'Sarah Johnson', title: 'Director of Marketing', company: 'Adobe', relationshipType: 'former_colleague', strength: 0.95, context: 'Worked together at TechCorp 2019-2021' },
...
app.get('/api/relationships/paths/:targetId', optionalAuth, async (req: AuthenticatedRequest, res: Response) => {
  const { targetId } = req.params;

  // Demo path data
  const demoPathsMap: Record<string, any> = {
    'mc-barker': {
      targetId: 'mc-barker',
...
app.post('/api/salesforce/export/:accountId', authenticate, async (req: AuthenticatedRequest, res: Response) => {
...
app.get('/api/salesforce/status', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  // Demo response
  res.json({
    success: true,
    connected: false,
