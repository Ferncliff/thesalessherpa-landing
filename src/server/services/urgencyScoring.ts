...
export function calculateUrgencyScore(
  ...
  const overall = Math.round(
    (timingScore.score / timingScore.maxScore * 100 * WEIGHTS.timing) +
  ...
  const allFactors: ScoringFactor[] = [
    ...timingScore.factors,
 ...
 */
function calculateTimingScore(account: Account, currentDate: Date): TimingScore {
  ...
  const closestQuarterEnd = quarterEndMonths.reduce((closest, qe) => 
    Math.abs(qe - month) < Math.abs(closest - month) ? qe : closest
  );
  ...
  const budgetNews = account.alerts?.filter(a => 
    a.type === 'news' && a.metadata?.keywords?.includes('budget')
  ) || [];
  ...
  const recentHighUrgency = account.alerts?.filter(a => 
 ...
 */
function calculateCompanySignalScore(account: Account): CompanySignalScore {
  const factors: ScoringFactor[] = [];
  let totalScore = 0;
  const maxScore = 100;
  ...
  if (fundingAlerts.length > 0) {
    const mostRecent = fundingAlerts.sort((a, b) => 
 ...
 * - Mutual connections
 */
function calculateRelationshipScore(account: Account): RelationshipScore {
  const factors: ScoringFactor[] = [];
  let totalScore = 0;
  const maxScore = 100;
  ...
  const decisionMakers = contacts.filter(c => 
    c.influence.budget >= 70 || c.influence.urgency >= 70
  );
  
  if (decisionMakers.length > 0) {
    const bestDM = decisionMakers.sort((a, b) => 
      (b.influence.budget + b.influence.urgency) - (a.influence.budget + a.influence.urgency)
    )[0];
 ...
 * - Meeting completion
 */
function calculateEngagementScore(account: Account, currentDate: Date): EngagementScore {
  const factors: ScoringFactor[] = [];
  let totalScore = 0;
  const maxScore = 100;
  ...
  if (account.lastActivityAt) {
    const daysSinceActivity = Math.floor(
      (currentDate.getTime() - new Date(account.lastActivityAt).getTime()) / (1000 * 60 * 60 * 24)
    );
  ...
  if (respondingContacts.length > 0) {
    const recentResponses = respondingContacts.filter(c => {
      const daysSince = Math.floor(
        (currentDate.getTime() - new Date(c.lastResponseAt!).getTime()) / (1000 * 60 * 60 * 24)
      );
      return daysSince <= 30;
    });
  ...
  const meetingActivities = activities.filter(a => 
    a.type.includes('meeting') || a.type.includes('call')
  );
  
  const recentMeetings = meetingActivities.filter(a => {
    const daysSince = Math.floor(
      (currentDate.getTime() - new Date(a.createdAt).getTime()) / (1000 * 60 * 60 * 24)
    );
    return daysSince <= 60;
  });
  ...
  const positiveEmails = emailActivities.filter(a => 
    a.outcome === 'replied' || a.outcome === 'opened'
  );
 ...
 * - Size compatibility
 */
function calculateFitScore(account: Account, icpProfile?: ICPProfile): FitScore {
  const factors: ScoringFactor[] = [];
  let totalScore = 0;
  const maxScore = 100;
  ...
  if (icpProfile.targetIndustries.length > 0) {
    const industryMatch = icpProfile.targetIndustries.some(i => 
      account.industry?.toLowerCase().includes(i.toLowerCase())
    );
  ...
  if (account.employeeCount) {
    const sizeMatch = 
      account.employeeCount >= icpProfile.minEmployees && 
    ...
    if (sizeMatch) {
      ...
      const distanceFromRange = Math.min(
        Math.abs(account.employeeCount - icpProfile.minEmployees),
        Math.abs(account.employeeCount - icpProfile.maxEmployees)
      );
  ...
  if (account.annualRevenue) {
    const revenueMatch = 
      account.annualRevenue >= icpProfile.minRevenue && 
  ...
  const titleMatches = contacts.filter(c => {
    ...
    return icpProfile.targetTitles.some(t => 
      c.influence.budget >= 60 || c.influence.technical >= 60
    );
  });
 ...
 */
function calculateCompetitiveScore(account: Account): CompetitiveScore {
  const factors: ScoringFactor[] = [];
  let totalScore = 0;
  const maxScore = 100;
  ...
  if (competitorAlerts.length > 0) {
    const urgentCompetitor = competitorAlerts.some(a => 
      a.urgency === 'critical' || a.urgency === 'high'
    );
  ...
  const techAlerts = alerts.filter(a => 
    a.type === 'technology_adoption' && 
  ...
  const evaluationSignals = alerts.filter(a => 
    a.metadata?.isEvaluation === true || 
  ...
  const dissatisfactionSignals = alerts.filter(a => 
    a.metadata?.sentiment === 'negative' ||
    a.metadata?.keywords?.some((k: string) => 
      ['frustrated', 'disappointed', 'switching', 'alternatives'].includes(k.toLowerCase())
    )
  );
 ...
 */
export function getPriorityLevel(urgencyScore: number): {
...
export async function batchRecalculateScores(
  ...
  const results = new Map<string, ScoringBreakdown>();
